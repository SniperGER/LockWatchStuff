...
-(void)renderIndicators {
    ...
    for (int i=0; i<12; i++) {
        float sinValue = sin(M_PI*((i+1)*30.0/180.0))*134.0;
        float cosValue = cos(M_PI*((i+1)*30.0/180.0))*134.0;
        
        UIView* hourIndicator = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 6, 42)];
        hourIndicator.backgroundColor = [self colorFromHexString:_accentColorIndicator];
        hourIndicator.center = CGPointMake((312/2)+sinValue, (390/2)-cosValue);
        hourIndicator.transform = CGAffineTransformMakeRotation(deg2rad((i+1)*30));
        hourIndicator.layer.cornerRadius = 3.0;
        hourIndicator.layer.allowsEdgeAntialiasing = YES;
        [indicatorContainer addSubview:hourIndicator];
    }
    
    for (int i=0; i<60; i++) {
        float sinValue = sin(M_PI*((i+1)*6.0/180.0))*152.0;
        float cosValue = cos(M_PI*((i+1)*6.0/180.0))*152.0;
        
        UIView* minuteIndicator = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 6, 6)];
        minuteIndicator.backgroundColor = [self colorFromHexString:_accentColorIndicator];
        minuteIndicator.center = CGPointMake((312/2)+sinValue, (390/2)-cosValue);
        minuteIndicator.transform = CGAffineTransformMakeRotation(deg2rad((i+1)*6));
        minuteIndicator.layer.cornerRadius = 3.0;
        minuteIndicator.layer.allowsEdgeAntialiasing = YES;
...
-(void) makeCustomizeSheet {
    ...
    customizeDetailLabel.text = @"COLOR";
    ...
    colorOptionsIndicator = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 400, 312, 90)];
    [customizeScrollView addSubview:colorOptionsIndicator];
    ...
    colorOptionsIndicator.contentSize = CGSizeMake(17*100, 90);
    for (int i=0; i<17; i++) {
        ...
        [colorOptionsIndicator addSubview:colorView];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (customizeScrollViewPager.currentPage == 0) {
        if (pageProgress > 0) {
            ...
            colorOptionsIndicator.alpha = 1-pageProgress;
            handContainer.alpha = 0;
    ...
    if (customizeScrollViewPager.currentPage == 1) {
        if (pageProgress > 0) {
            ...
            colorOptionsIndicator.alpha = pageProgress;
        } else if (pageProgress < 0) {
            handContainer.alpha = 1+pageProgress;
            colorOptions.alpha = 1+pageProgress;
            indicatorContainer.alpha = 0;
            colorOptionsIndicator.alpha = 0;
        } else {
            handContainer.alpha = 1;
    ...
    _scrollViewDelta = scrollView.contentOffset.x;
}

-(void) reRenderIndicators:(UITapGestureRecognizer*)sender {
    NSArray* colorHexArray = [NSArray arrayWithObjects:@"#AAAAAA", @"#E00A23", @"#FF512F", @"#FF9500", @"#FFEE31", @"#8CE328", @"#9ED5CC", @"#69C5DD", @"#18B5FC", @"#5F84BF", @"#997BF7", @"#B29AA6", @"#FF5963", @"#F4ACA5", @"#B08663", @"#AF9980", @"#D4B694", nil];
    _accentColorIndicator = [colorHexArray objectAtIndex:sender.view.tag-800];
    [self renderIndicators];
}
