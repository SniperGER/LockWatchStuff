...
//        defaults = [NSUserDefaults standardUserDefaults];
        ...
        //[preferences removeObjectForKey:@"AccentColor"];
...
- (void)makeCustomizeSheet {
    ...
    [[customizeScrollView subviews] makeObjectsPerformSelector:@selector(removeFromSuperview)];
    ...
    if ([[defaults objectForKey:@"watchColorSelector"] boolValue]) {
        detailOptions = [[LWWatchFaceCustomizations alloc] experimentalDetailCustomize:CGRectMake(0, 0, 312, 390) withCurrentDetailState:detailState withTarget:self withTapAction:@selector(reRenderIndicators:)];
    } else {
        detailOptions = [[LWWatchFaceCustomizations alloc] simpleDetailCustomize:CGRectMake(10, 10, 50, 370)
    ...
    // Page 2 - Accent Color
    if ([[defaults objectForKey:@"watchColorSelector"] boolValue]) {
        customizeSecondArm = [[LWWatchFaceCustomizations alloc] experimentalAccentColorCustomize:CGRectMake(312, 0, 312, 390) withAccentColor:accentColor withTarget:self andTapAction:@selector(reRenderSecondHand:)];
    } else {
        customizeSecondArm = [[LWWatchFaceCustomizations alloc] simpleAccentColorCustomize:CGRectMake(312, 0, 312, 390)
    ...
    // Page 3 - Date Options
    customizeDate = [[LWWatchFaceCustomizations alloc] generalDateCustomize:CGRectMake(312*2, 0, 312, 390)
                                                           withTarget:self
...
- (void)renderIndicators:(BOOL)customize {
    ...
    [indicatorContainer addSubview:[[LWWatchFaceIndicators alloc] simpleIndicators:detailState isCustomizing:customize]];
...
- (void)reRenderIndicatorsExperimental:(NSNumber*)newDetailState {
    //NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
    switch ([newDetailState intValue]) {
        case 800:
            detailState = 0;
            [preferences setValue:[NSNumber numberWithInt:0] forKey:@"DetailState"];
            break;
...
- (void)reRenderExperimental:(NSString*)newAccentColor {
    ...
    secondHand = [LWWatchFaceHands secondHand:newAccentColor];
    [secondHand.layer setPosition:CGPointMake(312/2, 390/2)];
    [secondHand setTransform:CGAffineTransformMakeRotation(deg2rad(180))];
    [handContainer addSubview:secondHand];
    
    [dateLabel setTextColor:[self colorFromHexString:newAccentColor]];
