...
- (void)deInitWatchFace:(BOOL)wasActiveBefore {
    ...
    [labelView setTransform:CGAffineTransformMakeScale(1, 1)];
...
- (void)makeCustomizeSheet {
    ...
    if ([[defaults objectForKey:@"watchColorSelector"] boolValue]) {
        colorSelector = [[LWWatchFaceCustomizations alloc] experimentalAccentColorCustomize:CGRectMake(0, 0, 312, 390) withAccentColor:accentColor withTarget:self andTapAction:@selector(reRenderExperimental:)];
        CGRect _frame1 = [[colorSelector subviews] objectAtIndex:1].frame;
        _frame1.origin.y = 356;
        [[colorSelector subviews] objectAtIndex:1].frame = _frame1;
        
        [[[colorSelector subviews] objectAtIndex:0] removeFromSuperview];
    } else {
        colorSelector = [[LWWatchFaceCustomizations alloc] colorIndicatorCustomize:CGRectMake(0, 0, 312, 390)
                                                                     withAccentColor:accentColor
                                                                          withTarget:self
                                                                       withTapAction:@selector(setLabelColor:)];
