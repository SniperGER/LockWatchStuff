...
#define CUSTOMIZE  [[NSBundle bundleWithPath:LocalizationsDirectory]localizedStringForKey:@"CUSTOMIZE" value:@"CUSTOMIZE" table:nil]
...
@synthesize plugins, startScaledDown, currentWatchFace, scaledDown;
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
            ...
            if ([stockSettings objectForKey:[plugin bundleIdentifier]] || [externalSettings objectForKey:[plugin bundleIdentifier]]) {
                if ([[stockSettings valueForKey:[plugin bundleIdentifier]] boolValue] || [[externalSettings valueForKey:[plugin bundleIdentifier]] boolValue]) {
                    ...
                    if (i == initialWatchFaceIndex) {
						...
						currentWatchFace = pluginInstance;
					} else {
						[pluginInstance setAlpha:0.0];
					}
        ...
        customizeButton = [[WatchButton alloc] initWithFrame:CGRectMake(0, 0, 152, 42) withTitle:CUSTOMIZE];
		...
		[[NSNotificationCenter defaultCenter] addObserver:self
												 selector:@selector(_scaleDown)
													 name:@"scrollview.requestEditMode"
												   object:nil];
...
- (void) scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
	...
	currentWatchFace = [watchFaceContainer objectAtIndex:page];
}
...
- (void)scaleUpToCustomize:(UITapGestureRecognizer*)sender {
    if (scaledDown) {
        scaledDown = NO;
		[[NSNotificationCenter defaultCenter] postNotificationName:@"scaledUp" object:self userInfo:nil];
		[[NSNotificationCenter defaultCenter] postNotificationName:@"scaledUpCustomize" object:self userInfo:nil];
        [scrollContainer setScaledDownScrollView:NO];
