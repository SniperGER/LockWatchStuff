...
- (id)initWithFrame:(CGRect)frame withWatchFaces:(NSArray*)watchFaces isScaledDown:(BOOL)startScaledDown {
    ...
    if (self) {
        ...
        if ([defaults objectForKey:@"initalWatchFaceIndex"]) {
            initialWatchFaceIndex = (int)[defaults integerForKey:@"initalWatchFaceIndex"];
        } else {
            [defaults setInteger:0 forKey:@"initalWatchFaceIndex"];
        ...
        for (int i=0; i<[watchFaces count]; i++) {
            ...
            WatchFaceBase* pluginInstance = [[[watchFaces objectAtIndex:i] principalClass] alloc];
            [pluginInstance setBundlePath:[NSString stringWithFormat:@"%@%@.watchface/", pluginLocationString, [plugin infoDictionary][@"CFBundleExecutable"]]];
            ...
            NSLog(@"%@", [plugin infoDictionary][@"CFBundleExecutable"]);
        ...
        /*for (int i=0; i<[watchFaces count]; i++) {
            ...
            NSBundle* pluginBundle = [watchFaces objectAtIndex:i];
            if ([pluginBundle localizedInfoDictionary]) {
                [[pluginInstance titleLabel] setText:[[NSString stringWithFormat:@"%@", [pluginBundle localizedInfoDictionary][@"CFBundleDisplayName"]] uppercaseString]];
            } else {
                [[pluginInstance titleLabel] setText:[[NSString stringWithFormat:@"%@", [pluginBundle infoDictionary][@"CFBundleDisplayName"]] uppercaseString]];
            }
            ...
            if ([stockSettings objectForKey:[pluginBundle bundleIdentifier]] || [externalSettings objectForKey:[pluginBundle bundleIdentifier]]) {
                if ([[stockSettings valueForKey:[pluginBundle bundleIdentifier]] boolValue] || [[externalSettings valueForKey:[pluginBundle bundleIdentifier]] boolValue]) {
                [watchFaceContainer addObject:pluginInstance];
                [watchFaceContainerDict setObject:pluginInstance forKey:[pluginBundle bundleIdentifier]];
                [scroll addSubview:pluginInstance];
                }
            }
        }
...
- (void) scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
    [defaults setInteger:page forKey:@"initalWatchFaceIndex"];
